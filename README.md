...
- **Voice Pipeline**:
  ...
  - **Continuous Hands-Free Voice Testing (`/voice/:workflowId`)**: Session remains active once started, automatically re-listening turn-by-turn after each AI audio response with smart silence detection (~1.5s auto-submit).
- **Hindi (`hi-IN`) & English (`en-US`) Multi-lingual Engine**:
  - Devanagari script and Hinglish keyword automatic language detection and natural speech response.
...
3. Set Environment Variable: `VITE_API_BASE_URL` = `https://your-render-backend-url.onrender.com`.
4. Click **Deploy**.
...
### `businesses`
Stores business profile documents.
- `id`: string
- `name`: string
- `industry`: string
- `phone`: string
...
### `workflows`
Stores intake configuration, system rules, and conditional logic.
- `id`: string
- `businessId`: string
- `title`: string
...
- `intakeFields`: Array<{ `name`, `label`, `type`, `required`, `options` }>
- `conditionalRules`: Array<{ `conditionField`, `operator`, `value`, `action` }>

### `calls`
Stores call history log records and subcollections (`responses`, `transcript`).
- `id`: string
- `workflowId`: string
- `businessId`: string
...
2. **Real-time WebSockets (Pipecat / WebRTC)**: Transition from REST audio blob upload to bidirectional streaming WebSockets for sub-500ms voice latency.
3. **SMS & Email Notification Triggers**: Integrate Twilio SMS / SendGrid API to execute conditional rule actions (`send_sms`, `flag_urgent`).
